...
import { CompPostLocation, createComment } from "@/model/companion";

type HeadProps = {
  parent: CompPostLocation & { creator?: user };
  userId: number;
};

export function CompHeader({ parent, userId }: HeadProps) {
...
type CompReplyComponentProps = {
  discussionId: number;
  replyData: companion_comment & {
    replies?: companion_comment[];
    creator?: user;
  };
  userId: number;
  ...
  return (
    <div className="mt-4 pl-4 border-l-2 border-gray-700 w-full">
      ...
      <div>
        {replyData?.creator ? (
          <User
            name={replyData.creator.username}
            description={replyData.creator.firstName}
            className="mt-1"
          />
        ) : (
